...
class MemberMatchVC: BaseViewController {
    
    var showTop: ShowTop2?
    
    lazy var tableView: MyTable2VC<MemberMatchCell, MatchTeamTable, MemberMatchVC> = {
        let tableView = MyTable2VC<MemberMatchCell, MatchTeamTable, MemberMatchVC>(selectedClosure: tableViewSetSelected(row:), getDataClosure: getDataFromServer, myDelegate: self, isRefresh: false)
    ...
    func getDataFromServer(page: Int) {
        ...
        dataService.getList(params: Member.instance.token, page: page, perPage: tableView.perPage) { (success) in
...
class MemberMatchCell: BaseCell<MatchTeamTable, MemberMatchVC> {
    ...
    override func configureSubViews() {
        ...
        titleLbl.text = item?.matchTable?.title
