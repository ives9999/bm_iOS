...
extension UIImageView {
    ...
    func downloaded(from url: URL, isCircle: Bool = true, rounded: CGFloat = 0, contentMode mode: UIView.ContentMode = UIView.ContentMode.scaleAspectFit) {
        contentMode = mode
        URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            if isCircle {
                image = image1.circleMasked ?? image
            } else {
                if rounded > 0 {
                    image = image1.rounded(radius: rounded)
                }
            }
    ...
    func downloaded(from link: String, isCircle: Bool = true, rounded: CGFloat = 0, contentMode mode: UIView.ContentMode = UIView.ContentMode.scaleAspectFit) {
        //print(link)
        guard let url = URL(string: link) else {return}
        downloaded(from: url, isCircle: isCircle, rounded: rounded, contentMode: mode)
    }
